...
export type OllamaEmbedConfig = {
  /** Ollama server URLs (default: localhost:11434, override via QMD_OLLAMA_URLS env) */
  urls?: string[];
  /** Model name on Ollama (default: embeddinggemma, override via QMD_OLLAMA_MODEL env) */
  model?: string;
...
/**
 * Embedding backend using Ollama HTTP API.
 ...
 *
 * Configure multiple servers via QMD_OLLAMA_URLS environment variable:
 *   QMD_OLLAMA_URLS=http://gpu1:11434,http://gpu2:11434,http://localhost:11434
 *
...
export class OllamaEmbed {
  ...
  constructor(config: OllamaEmbedConfig = {}) {
    this.urls = config.urls
      ?? (process.env.QMD_OLLAMA_URLS
        ? process.env.QMD_OLLAMA_URLS.split(",").map((u) => u.trim())
        : DEFAULT_OLLAMA_URLS);
    this.model = config.model ?? (process.env.QMD_OLLAMA_MODEL || DEFAULT_OLLAMA_MODEL);
  ...
  private async probeAll(): Promise<void> {
    ...
    if (this.activeUrls.length === 0) {
      throw new Error(
        `No reachable Ollama server. Tried: ${this.urls.join(", ")}`
      );
    }
    ...
    const worker = async (url: string) => {
      while (true) {
        ...
        try {
          ...
          process.stderr.write(`\nOllama embed: ${url} failed, removing from pool\n`);
      ...
      while (true) {
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
          try {
            ...
            if (attempt < MAX_RETRIES) {
              process.stderr.write(`\nOllama embed: ${name} attempt ${attempt}/${MAX_RETRIES} failed: ${msg} — retrying...\n`);
              await new Promise((r) => setTimeout(r, 1000 * attempt));
            } else {
              process.stderr.write(`\nOllama embed: ${name} failed after ${MAX_RETRIES} attempts: ${msg}\n`);
              consecutiveFailures++;
            }
          }
        }
        ...
        if (!success) {
          ...
          if (consecutiveFailures >= 2) {
            process.stderr.write(`\nOllama embed: removing ${name} (${consecutiveFailures} consecutive failures)\n`);
            this.activeUrls = this.activeUrls.filter((u) => u !== url);
            break;
          }
        }
...
// =============================================================================
// Singleton for default OllamaEmbed instance
// =============================================================================

let defaultOllamaEmbed: OllamaEmbed | null = null;

/**
 * Get the default OllamaEmbed instance.
 * Checks if Ollama is likely available (QMD_OLLAMA_URLS set or localhost:11434 reachable).
 */
export function getDefaultOllamaEmbed(): OllamaEmbed {
  if (!defaultOllamaEmbed) {
    defaultOllamaEmbed = new OllamaEmbed();
  }
  return defaultOllamaEmbed;
}

export function setDefaultOllamaEmbed(embedder: OllamaEmbed | null): void {
  defaultOllamaEmbed = embedder;
}

/** Print per-server stats from the default OllamaEmbed instance */
export function printEmbedderStats(): void {
  if (defaultOllamaEmbed) {
    defaultOllamaEmbed.printStats();
  }
}
